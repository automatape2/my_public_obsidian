...
`;
  ...
  static createTaskNote(path, task, template, notename, app) {
    return __async(this, null, function* () {
      let filename = notename.replace(/{{TASK_ID}}/g, task.id).replace(/{{TASK_STATE}}/g, task.state).replace(/{{TASK_TYPE}}/g, task.type.replace(/ /g, "")).replace(/{{TASK_ASSIGNEDTO}}/g, task.assignedTo);
      const filepath = path + `/${filename}.md`;
    ...
    return __async(this, null, function* () {
      ...
      try {
        if (settings.jiraSettings.mode == "sprints") {
          const sprintsResponse = yield (0, import_obsidian3.requestUrl)({
            method: "GET",
            headers,
